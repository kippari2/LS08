...
Player.waklingSpeedDefault = Player.walkingSpeed;
...
function Player.create(posX, yOffset, posZ, rotX, rotY)
    Player.playerName = loadI3DFile("data/templates/playermod.i3d");
    ...
    if getNumOfChildren(Player.camera) > 0 then
        ...
        Player.zoomCamera = getChild(Player.camera, "zoomCamera", 0); --There is no setFovy in LS08, so we need a separate camera for this
        if Player.zoomCamera == 0 then
            print("Error: missing zoom camera")
        end;
    end;

    Player.camX, Player.camY, Player.camZ = getTranslation(Player.camera);
    Player.camDefaultY = Player.camY;
...
function Player.update(dt)
    ...
    --The value is unlimited for lulz, go crazy with it! :D
    if Input.isKeyPressed(Input.KEY_KP_plus) then
        Player.walkingSpeed = Player.walkingSpeed+0.0002;
    end;

    if Input.isKeyPressed(Input.KEY_KP_minus) and Player.walkingSpeed > 0.0012 then
        Player.walkingSpeed = Player.walkingSpeed-0.0002;
    end;

    if Input.isKeyPressed(Input.KEY_KP_multiply) then
        Player.walkingSpeed = Player.waklingSpeedDefault;
    end;

    if Input.isKeyPressed(Input.KEY_pageup) then
        Player.camY = Player.camY+0.1;
    end;

    if Input.isKeyPressed(Input.KEY_pagedown) then
        Player.camY = Player.camY-0.1;
    end;

    if Input.isKeyPressed(Input.KEY_home) then
        Player.camY = Player.camDefaultY;
    end;

    if Input.isKeyPressed(Input.KEY_space) and Player.zoomCamera ~= 0 then
        setCamera(Player.zoomCamera);
    else
    ...
    renderText(0.842, 0.962, 0.02, string.format("Speed: %.2f", Player.walkingSpeed*100));
end;
