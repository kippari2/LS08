...
CombineAP3 = {};
source("data/scripts/vehicles/fmz/Bizon3.lua");

function CombineAP3:new(configFile, positionX, offsetY, positionZ, rotationY, customMt)
    if CombineAP3_mt == nil then
        CombineAP3_mt = Class(CombineAP3, Bizon3);
    end;
    ...
    local instance = Bizon3:new(configFile, positionX, offsetY, positionZ, rotationY, mt); ---
...
function CombineAP3:delete()
    CombineAP3:superClass().delete(self);
end;

function CombineAP3:mouseEvent(posX, posY, isDown, isUp, button)
    CombineAP3:superClass().mouseEvent(self, posX, posY, isDown, isUp, button);
end;

function CombineAP3:keyEvent(unicode, sym, modifier, isDown)
    CombineAP3:superClass().keyEvent(self, unicode, sym, modifier, isDown);
...
function CombineAP3:update(dt)
    CombineAP3:superClass().update(self, dt);
...
function CombineAP3:draw()
    CombineAP3:superClass().draw(self);
...
function CombineAP3:onEnter()
    CombineAP3:superClass().onEnter(self);
    stopSample(self.fulledSound);
end;

function CombineAP3:onLeave()
    CombineAP3:superClass().onLeave(self);
    
    if self.pipeLight ~= nil then
        setVisibility(self.pipeLight, false);
    end;
end;
function CombineAP3:onDetach()
    CombineAP3.onDetach(self);
    ---stopSample(self.fulledSound);
