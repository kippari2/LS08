...
function Bizon2:new(configFile, positionX, positionY, positionZ, yRot, customMt)

    if Bizon2_mt == nil then
        Bizon2_mt = Class(Bizon2, Vehicle);
    end;
    ...
    local instance = Bizon2:superClass():new(configFile, positionX, positionY, positionZ, yRot, mt);
	...
	instance.blindParticleActive = true;
...
function Bizon2:delete()
    ...
    Bizon2:superClass().delete(self);

end;

function Bizon2:keyEvent(unicode, sym, modifier, isDown)
    Bizon2:superClass().keyEvent(self, unicode, sym, modifier, isDown);
	if self.isEntered then
		if isDown and sym == self.keys.stroh then
			self.hayOn = not self.hayOn;
		end;
		if isDown and sym == self.keys.chopperblind then --and getRotation(self.strawChopperNode2) == self.chopperMin2[1] 
...
function Bizon2:update(dt, isActive)
    Bizon2:superClass().update(self, dt, isActive);
	...
	if self.strawChopperNode1 ~= nil and self.strawChopperNode2 ~= nil then
	...
	if not self.hayOn and self.currentRot1[1] ~= self.chopperMin1[1] and self.currentRot2[1] == self.chopperMin2[1] and self.attachedCutter ~= nil and self.attachedCutter.reelStarted and self.attachedCutter.lastArea > 0 then
...
function Bizon2:attachCutter(cutter)
    if self.attachedCutter == nil then
        -- attach
        local jointDesc = self.cutterAttacherJoint;
...
function Bizon2:detachCurrentCutter()
...
function Bizon2:setGrainTankFillLevel(fillLevel)
...
function Bizon2:findTrailerRaycastCallback(transformId, x, y, z, distance)
...
function Bizon2:onLeave()
    Bizon2:superClass().onLeave(self);
    _=[[if self.pipeLight ~= nil then
...
function Bizon2:draw()
    Bizon2:superClass().draw(self);
    ...
    if self.drawFillLevel then
        ...
        Bizon2:superClass().drawGrainLevel(self, self.grainTankFillLevel, self.grainTankCapacity, 95);
    end;
...
function Bizon2:startThreshing()
    if self.attachedCutter ~= nil then
