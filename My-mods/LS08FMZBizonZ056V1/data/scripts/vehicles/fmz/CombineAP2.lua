...
CombineAP2 = {};
--data/scripts/vehicles/fmz/Bizon2.lua
source("data/scripts/vehicles/fmz/Bizon2.lua");

function CombineAP2:new(configFile, positionX, offsetY, positionZ, rotationY, customMt)
    if CombineAP2_mt == nil then
        CombineAP2_mt = Class(CombineAP2, Bizon2);
    end;
    ...
    local instance = Bizon2:new(configFile, positionX, offsetY, positionZ, rotationY, mt); ---
...
function CombineAP2:delete()
    CombineAP2:superClass().delete(self);
end;

function CombineAP2:mouseEvent(posX, posY, isDown, isUp, button)
    CombineAP2:superClass().mouseEvent(self, posX, posY, isDown, isUp, button);
end;

function CombineAP2:keyEvent(unicode, sym, modifier, isDown)
    CombineAP2:superClass().keyEvent(self, unicode, sym, modifier, isDown);
...
function CombineAP2:update(dt)
    CombineAP2:superClass().update(self, dt);
...
function CombineAP2:draw()
    CombineAP2:superClass().draw(self);
...
function CombineAP2:onEnter()
    CombineAP2:superClass().onEnter(self);
    stopSample(self.fulledSound);
end;

function CombineAP2:onLeave()
    CombineAP2:superClass().onLeave(self);
    
    if self.pipeLight ~= nil then
        setVisibility(self.pipeLight, false);
    end;
end;
function CombineAP2:onDetach()
    CombineAP2.onDetach(self);
    ---stopSample(self.fulledSound);
