...
function Bizon1:new(configFile, positionX, positionY, positionZ, yRot, customMt)

    if Bizon1_mt == nil then
        Bizon1_mt = Class(Bizon1, Vehicle);
    end;
    ...
    local instance = Bizon1:superClass():new(configFile, positionX, positionY, positionZ, yRot, mt);
	...
	instance.hoodNode = Utils.indexToObject(instance.rootNode, getXMLString(xmlFile, "vehicle.hood#index"));
	...
	instance.automaticCutterLift = getXMLBool(xmlFile, "vehicle.automaticCutterLift");
	--instance.showRailing2 = getXMLBool(xmlFile,"vehicle.railing2#showOnLoad", true);

    delete(xmlFile);

	instance.coversVisible = false;
	instance.hoodVisible = false;
...
function Bizon1:delete()
    ...
    Bizon1:superClass().delete(self);

end;

function Bizon1:keyEvent(unicode, sym, modifier, isDown)
    Bizon1:superClass().keyEvent(self, unicode, sym, modifier, isDown);
	if self.isEntered then
		...
		if isDown and sym == self.keys.hood then
			self.hoodVisible = not self.hoodVisible;
			if self.hoodNode ~= nil then
				setVisibility(self.hoodNode, self.hoodVisible);
			end;
		end;
...
function Bizon1:update(dt, isActive)
    Bizon1:superClass().update(self, dt, isActive);
...
function Bizon1:attachCutter(cutter)
    if self.attachedCutter == nil then
        -- attach
        local jointDesc = self.cutterAttacherJoint;
...
function Bizon1:detachCurrentCutter()
...
function Bizon1:setGrainTankFillLevel(fillLevel)
...
function Bizon1:findTrailerRaycastCallback(transformId, x, y, z, distance)
...
function Bizon1:onLeave()
    Bizon1:superClass().onLeave(self);
    _=[[if self.pipeLight ~= nil then
...
function Bizon1:draw()
    Bizon1:superClass().draw(self);
    ...
    if self.drawFillLevel then
        ...
        Bizon1:superClass().drawGrainLevel(self, self.grainTankFillLevel, self.grainTankCapacity, 95);
    end;
	...
	if self.help then
		...
		if self.rundumleuchten ~= nil then
			renderText(0.05, 0.26, 0.025, string.format("Beacon on/off: %s", self.keynames.beacon));
		end;
		if self.hoodNode ~= nil then
			renderText(0.05, 0.24, 0.025, string.format("Hood on/off: %s", self.keynames.hood));
		end;
		if self.coversNode ~= nil then
...
function Bizon1:startThreshing()
    if self.attachedCutter ~= nil then
